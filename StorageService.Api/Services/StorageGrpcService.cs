...
public class StorageGrpcService : StorageService.Api.Protos.StorageService.StorageServiceBase
{
    ...
    private readonly IUploadEventProducer _producer;
    public StorageGrpcService(IMediator mediator, IUploadEventProducer producer) { _mediator = mediator;
        _producer = producer;
    }
    ...
    public override async Task<UploadReply> UploadByKafka(
       UploadRequest request,
       ServerCallContext context)
    {
        var fileId = Guid.NewGuid().ToString();

        await _producer.PublishAsync(new FileUploadRequestedEvent
        {
            Id = fileId,
        ...
        // Immediate ACK
        return new UploadReply { Id = fileId };
    }
